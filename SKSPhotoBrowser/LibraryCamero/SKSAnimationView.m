...
@protocol SKSAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(SKSAnimationView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface SKSAnimationView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<SKSAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end

@implementation SKSAnimationView
...
- (void)setAnimatedImage:(SKSAnimationImage *)animatedImage
{
...
{
    // Presision is set to half of the `kSKSAnimationImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kSKSAnimationImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            SKSWeakProxy *weakProxy = [SKSWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    // Early return!
    if (!self.shouldAnimate) {
        FLLog(SKSLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber) {
        ...
        if (image) {
            FLLog(SKSLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            FLLog(SKSLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
